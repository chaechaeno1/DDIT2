...
//		String addr = scan.next();
		...
		/*
		 *
		 - Scanner객체는 입력 버퍼를 이용하여 입력 작업을 수행
		 	즉, 입력 버퍼를 확인하여 값이 있으면 현재 입력 버퍼에 있는 값을 가져가고
		 	없으면 새로운 데이터를 입력받은 후 가져간다.
		
		 1. Scanner의 next(), nextInt(), nextDouble() 등 (nextLine()을 제외한 메서드)
		 	==> 이 메서드들은 사이띄기, tab키, Enter키를 구분 문자로 분리해서 분리된 자료만 읽음
		 2. Scanner의 nextLine()메서드
