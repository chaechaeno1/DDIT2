...
/*
 	...
 	- DatagramSocket객체 ==> 데이터의 송수신과 관련된 작업을 수행. (우체부)
 	- DatagramPacket객체 ==> 주고 받는 실제 데이터와 관련된 작업을 수행. (소포)
	...
	public static void main(String[] args) {
		try {
			...
			while (true) {
				...
				// 상대방이 보낸 메시지 출력하기
				// 수신용 패킷 객체.getData() ==> 실제 수신한 데이터를 byte형 배열로 반환
...
//				String msg = new String(bMsg, 0, inpacket.getLength(), "utf-8");
				...
				// 송신용 패킷 객체 생성
				// ==> 전송할 데이터가 저장된 byte형 배열, 전송할 자료의 길이(배열의 길이), 상대방의 주소 정보, 포트번호
				// 이렇게 4가지를 지정하여 객체를 생성

				outpacket = new DatagramPacket(sendMsg, sendMsg.length, address, port);

				// 데이터 송신하기 ==> 소켓 객체의 send()메서드
