...
public class LprodMybatisTest {
	...
	public static void main(String[] args) {
		...
		System.out.print("(1) Lprod_id 입력 >> ");
		int lprodId = scan.nextInt();

		System.out.print("(2) Lprod_gu 입력 >> ");
		String lprodGu = scan.next();

		System.out.print("(3) Lprod_nm 입력 >> ");
		...
		try {
			...
			// 형식) SqlSession객체.insert("namespace속성값.실행할태그의 id 속성값", 파라미터 클래스)
			// 반환값 : 작업에 성공한 레코드 수
			int insertCnt = session.insert("lprod.insertLprod", lprodVo);

			if (insertCnt > 0) {
				// 4) SqlSesstion객체를 생성할 때 AutoCommit이 비활성화된 상태일 때는
				// commit을 직접 실행해야 한다.
				session.commit();
				System.out.println();
